...
const ClientCarousel: React.FC = () => {
  ...
  return (
    <section className="relative overflow-hidden py-8 md:py-10 bg-black">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-6 md:mb-8">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2 bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">
        ...
        {/* Carousel wrapper */}
        <div className="relative overflow-hidden bg-white/[0.02] backdrop-blur-md py-4 md:py-6 border border-gray-800/50">
          {/* Fade gradients */}
          <div className="absolute left-0 top-0 bottom-0 w-12 md:w-16 bg-gradient-to-r from-black to-transparent z-10 pointer-events-none"></div>
          <div className="absolute right-0 top-0 bottom-0 w-12 md:w-16 bg-gradient-to-l from-black to-transparent z-10 pointer-events-none"></div>

          {/* Carousel container */}
          <div
            ref={carouselRef}
            className="flex overflow-hidden px-12 md:px-16 gap-6 sm:gap-8 md:gap-10 smooth-scroll"
          ...
          >
            {duplicatedLogos.map((client, index) => (
              <div
                key={`${client.id}-${index}`}
                className="flex-shrink-0 flex items-center justify-center w-28 h-20 sm:w-32 sm:h-24 md:w-36 md:h-28 transition-all duration-300 ease-out hover:scale-110"
              >
                <Image
                  src={client.logo}
                  alt={client.name}
